...
// FIX: Removed Schema type annotation as it is deprecated.
...
export const generateSingleScene = async (
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      // FIX: Corrected the structure of the `contents` payload.
...
export const generateStoryFromImages = async (
  ...
  const scenePromises = images.map(async (image, index) => {
    ...
    // FIX: Corrected the return object to match the Scene interface.
    // Changed `mediaUrl` to `mediaUrls` and wrapped it in an array.
    // Removed `isAiGenerated` as it's not part of the Scene type.
    return {
...
export const refineText = async (
  ...
  try {
    const response = await ai.models.generateContent({
      model: "gemini-2.5-flash",
      // FIX: Corrected the structure of the `contents` payload for simple text prompts.
      contents: prompt,
      config: {
        temperature: 0.3, 
      },
